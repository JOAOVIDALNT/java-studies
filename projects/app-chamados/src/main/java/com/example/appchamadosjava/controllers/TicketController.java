...
public class TicketController {
    ...
    @PostMapping
    public ResponseEntity<TicketDTO> create(String email, @Valid @RequestBody TicketDTO ticketDTO) {
        return new ResponseEntity<>(ticketService.create(email, ticketDTO), HttpStatus.CREATED);
    }
    ...
    public ResponseEntity<TicketStatusUpdateDTO> updateStatus(@PathVariable Long id, @Valid @RequestBody TicketStatusUpdateDTO ticketStatusUpdateDTO) {
        return new ResponseEntity<>(ticketService.updateStatus(id, ticketStatusUpdateDTO), HttpStatus.OK);
    }

    @PutMapping("/problem/{id}")
    public ResponseEntity<TicketProblemUpdateDTO> updateProblem(@PathVariable Long id, @Valid @RequestBody TicketProblemUpdateDTO ticketProblemUpdateDTO) {
        return new ResponseEntity<>(ticketService.updateProblem(id, ticketProblemUpdateDTO), HttpStatus.OK);
    }

    @PutMapping("/review/{id}")
    public ResponseEntity<TicketAddReviewDTO> addReview(@PathVariable Long id, @Valid @RequestBody TicketAddReviewDTO ticketAddReviewDTO) {
    ...
    public ResponseEntity<List<TicketFindDTO>> findByUser(@PathVariable String email) {
        return new ResponseEntity<>(ticketService.findTicketsByUser(email), HttpStatus.OK);
    }
}
